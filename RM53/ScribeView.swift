...
class ScribeView: UIView
{
    ...
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?)
    {
        timer =  Timer.scheduledTimer(timeInterval: 0.3,
            target: self,
            selector: #selector(ScribeView.timerHandler),
            userInfo: nil,
            repeats: false)
    }
    ...
    func handleGesture()
    {
        ...
        var cells = [[Bool]](repeating: [Bool](repeating: false, count: cellCount), count: cellCount)
...
{
    func scribeView(_ scribeView: ScribeView, didMatchPattern: String)
}
